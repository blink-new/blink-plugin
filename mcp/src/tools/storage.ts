...
export const storageTools = {
  blink_storage_list: {
    ...
    execute: async (input: { projectId: string; prefix?: string }) => {
      ...
      return resourcesRequest(`/api/storage/${input.projectId}/list?${params}`)
    },
  },
  blink_storage_url: {
    ...
    execute: async (input: { projectId: string; path: string }) =>
      resourcesRequest(`/api/storage/${input.projectId}/public-url?path=${encodeURIComponent(input.path)}`),
  },
  blink_storage_delete: {
    description: 'Delete a file from storage',
    inputSchema: z.object({ projectId: z.string(), path: z.string() }),
    execute: async (input: { projectId: string; path: string }) =>
      resourcesRequest(`/api/storage/${input.projectId}/remove`, { method: 'DELETE', body: { paths: [input.path] } }),
  },
}
